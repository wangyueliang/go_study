...
// 原文链接：https://blog.csdn.net/pengpengzhou/article/details/107384861

func ms_test() { //https://blog.csdn.net/pengpengzhou/article/details/107384861

	x := []string{"abcdef", "bcd", "cde", "00", "11"} // 便利切片  输出  k,v
	for v, k := range x {
		fmt.Println("v:", v, " k:", k)
	}
	...
	filec, err := os.Open("test.go")
	defer filec.Close()
	//不会触发panic。file即便是nil Close()也只是会返回错误。
	if err != nil {
...
func main() {
	...
	*/
	...
	fmt.Println("当前 goroutines:", runtime.NumGoroutine())

	// bufio   rtmp 协议 读取 看看
	// 创建一个socket的读取器  来自 go  语言从入门到进阶 p12章节 tcppkt
	//dataReader := bufio.NewReader(conn)

	//*bufio.ReadWriter// livego  就是这么读取的 tcp数据   Flush()   刷新数据

	// 发送队列sendChan chan interface{}  //接口类型的队列

	// switch ev := raw.(type) { //      接口.(type)是结构体类型
	ms_test()
